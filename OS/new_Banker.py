...
def print_need_matrix(need_matrix):
    print("\nNeed Matrix:")
    header = ["R{}".format(i) for i in range(len(need_matrix[0]))]
    print("{:10} {}".format("", " ".join(header)))
    for i in range(len(need_matrix)):
        print("{:10} {}".format(
            f"P{i}",
            " ".join("{:>2}".format(str(need_matrix[i][j])) for j in range(len(need_matrix[0])))
        ))
...
def main():
    ...
    if len(safe_sequence) == len(allocation_matrix):
        ...
        # Print Need Matrix
        print_need_matrix(calculate_need_matrix(allocation_matrix, max_matrix))
        
        # Print Safe sequence
        print("Safe sequence:", ", ".join("P{}".format(i) for i in safe_sequence))
        
        print("\nSystem is in a safe state.")
        print(f"The resource allocation has been completed with {iteration} iteration(s).")
    else:
        print(f"deadlock occured at {iteration} iteraion(s)")
